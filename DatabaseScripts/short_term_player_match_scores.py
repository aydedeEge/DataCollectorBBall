...
def calculate(player_id):
    ...
    if(len(result_set) != 0):
        command = "UPDATE `d2matchb_bball`.`player_matches` SET short_score = CASE player_match_id "
        command += when_conditional + "ELSE short_score END WHERE player_match_id IN(" + when_list[:-1] + ");"
...
def get_player_ids(limit):
    connection = MySQLdb.connect(host = os.environ["host"],    # your host, usually localhost
                                 ...
                                 db = os.environ["db"])        # name of the data base
    cursor = connection.cursor(MySQLdb.cursors.DictCursor)
    command = "SELECT distinct(pid) FROM player_matches WHERE pid NOT IN (SELECT pid FROM player_matches WHERE short_score is not null) limit " + str(limit) + ";"
    cursor.execute(command)
    result_set = cursor.fetchall()
    result = []
    for row in result_set:
