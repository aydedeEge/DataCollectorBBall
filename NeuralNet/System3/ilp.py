...
def ilp(expectedScores, playerCosts, playerPositions, globalBudget):
    ...
    for i in range(1, numberOfPlayers):
        my_lp_problem += my_lp_problem.objective + expectedScores[i] * decisionVariables["x" + str(
            i)]
    ...
    for i in range(1, 6):
        ...
        for index in indecesOfPosition:
            if (isFirstIteration):
                ...
                if (i == 1):
                    my_lp_problem += decisionVariables[
                        "x" + str(index)] == 1, "Constraint_" + str(i)
                else:
                    my_lp_problem += decisionVariables[
                        "x" + str(index)] == 2, "Constraint_" + str(i)

                isFirstIteration = False
            else:
                my_lp_problem.constraints["Constraint_" + str(
                    i)] += decisionVariables["x" + str(index)]

    # Constraint 6:
    my_lp_problem += playerCosts[0] * decisionVariables["x" + str(
        0)] <= globalBudget, "Constraint_6"
    for i in range(1, numberOfPlayers):
        my_lp_problem.constraints["Constraint_6"] += playerCosts[
            i] * decisionVariables["x" + str(i)]
